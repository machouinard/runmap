-- Add toggle to include/exclude alleys and rebuild dependent matviews accordingly
-- Usage:
--   -- exclude alleys (default)
--   UPDATE runmap.settings SET include_alleys = false WHERE id = 1;
--   SELECT runmap.refresh_after_settings();
--   -- include alleys later
--   UPDATE runmap.settings SET include_alleys = true WHERE id = 1;
--   SELECT runmap.refresh_after_settings();
...
-- 1) Toggle column on settings
ALTER TABLE runmap.settings
ADD COLUMN IF NOT EXISTS include_alleys boolean NOT NULL DEFAULT false;

-- 2) Recreate streets_unrun with partial-difference + dedup and alley toggle
...
WITH aoi AS (
  ...
  WHERE s.geom && bbox.g
    AND (
      ...
      OR (
        (SELECT include_alleys FROM runmap.settings LIMIT 1)
        AND s.highway = 'service' AND COALESCE(s.service,'') = 'alley'
      )
    )
), diff AS (
  SELECT
...
-- 3) Recreate coverage_stats aligned to the same base with alley toggle
...
-- 4) Convenience helper: refresh views after changing settings
CREATE OR REPLACE FUNCTION runmap.refresh_after_settings()
RETURNS void
LANGUAGE plpgsql
AS $$
BEGIN
  PERFORM pg_advisory_xact_lock(hashtext('runmap.refresh_after_settings'));

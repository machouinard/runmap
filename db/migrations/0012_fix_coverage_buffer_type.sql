-- Fix: coverage_buffer_m can be MULTIPOLYGON; change type and make robust
...
-- Recreate coverage_buffer_m as MULTIPOLYGON(4326) and always ST_Multi
...
SELECT
  ...
  ST_Multi(
    ST_Transform(
      ST_Buffer(g, (SELECT buffer_m FROM p)::double precision),
      4326
    )
...
-- Recreate streets_unrun and coverage_stats (identical logic, new dependency)

#!/bin/bash
# Parse GPX and insert into database
...
gpx_file = "$GPX_FILE"
route_name = os.environ.get('ROUTE_NAME', None)
...
try:
    ...
    if table_name == 'walks_raw':
        insert_sql = f"""
            INSERT INTO runmap.{table_name} (filename, geom, workout_hash, start_time, duration_seconds, distance_km, total_distance_m, avg_pace_min_per_mile, route_name)
            VALUES (%s, ST_GeomFromText(%s, 4326), %s, %s, %s, %s, %s, %s, %s)
            ON CONFLICT (workout_hash) DO NOTHING
            RETURNING id
        """
        cur.execute(insert_sql, (filename, wkt, workout_hash, start_time, duration_seconds, distance_km, total_distance_m, avg_pace_min_per_mile, route_name))
    elif table_name == 'runs_raw':
        insert_sql = f"""
            INSERT INTO runmap.{table_name} (filename, geom, workout_hash, start_time, duration_seconds, distance_km, total_distance_m, metadata, route_name)
            VALUES (%s, ST_GeomFromText(%s, 4326), %s, %s, %s, %s, %s, %s::jsonb, %s)
            ON CONFLICT (workout_hash) DO NOTHING
            RETURNING id
        """
        cur.execute(insert_sql, (filename, wkt, workout_hash, start_time, duration_seconds, distance_km, total_distance_m, metadata_json, route_name))
    else:  # cycling_raw
        insert_sql = f"""
            INSERT INTO runmap.{table_name} (filename, geom, workout_hash, start_time, duration_seconds, distance_km, total_distance_m, route_name)
            VALUES (%s, ST_GeomFromText(%s, 4326), %s, %s, %s, %s, %s, %s)
            ON CONFLICT (workout_hash) DO NOTHING
            RETURNING id
        """
        cur.execute(insert_sql, (filename, wkt, workout_hash, start_time, duration_seconds, distance_km, total_distance_m, route_name))
